...
/**
 * <p>
 * Default Manager implementation of for class
 * {@link com.helger.aufnahme.businessobj.ExBiotopbaumBO}
 * </p>
 * <p>
 ...
 */
@ThreadSafe
public class ExBiotopbaumBOManager extends AbstractPhotonMapBasedWALDAO <IExBiotopbaumBO, ExBiotopbaumBO>
{

  public ExBiotopbaumBOManager (@Nullable final String sFilename) throws DAOException
  {
    super (ExBiotopbaumBO.class, sFilename);
  }

  public ExBiotopbaumBOManager (@Nullable final String sFilename,
                                @Nonnull final AbstractMapBasedWALDAO.InitSettings <ExBiotopbaumBO> aInitSettings) throws DAOException
  {
   ...
   * @param aType
   *        Biotopbaum-Typ (Hauptauswahlkriterium). May neither be
   ...
   * @param sHanglage
   *        Angabe von Neigungen: keine, Angabe von Neigungen, Freitext. May be
   *        <code>null</code>.
   * @param bEinschichtig
   *        Wald, einschichtig (1 Baumschicht, kaum Unterwuchs) oder
   *        mehrschichtiger Bestand (Unterwuchs, Strauchsch., evtl. 2.
   *        Baumschicht).
   * @param bSolitary
   *        Solitärbaum.
   ...
   * @param sBeschreibung
   *        Bemerkungen/ (ausführliche) Beschreibung. May not be
   *        <code>null</code>.
   * @param eTreeKind
   *        Baumart laut Aufnahmeblatt. May not be <code>null</code>.
   * @param aCaves
   *        Höhlentyp und Anzahl mit Klasseneinteilung (Checkbox, mehrere
   *        möglich; dann noch Klassenangaben; C22-35 im Aufnahmemanual). May
   *        not be <code>null</code>.
   * @param aTrunk
   *        Daten zum Stamm/zu den Stämmen. May neither be <code>null</code> nor
   *        empty.
   * @param eVitality
   *        Vitalität. May not be <code>null</code>.
   * @param aSpecialStructure
   *        Baum-Sonderstrukturen (Mehrfachnennung möglich). May not be
   *        <code>null</code>.
   * @param sOtherSpecial
   *        Sonstige Sonderstrukturen inkl. Beschreibung. May be
   *        <code>null</code>.
   * @param bAspirant
   *        Anwärter in nächster Umgebung (Potenzial für Habitatbaumgruppe).
   * @param sAspirantDesc
   *        Beschreibung Anwärter in nächster Umgebung (Potenzial für
   *        Habitatbaumgruppe). May be <code>null</code>.
   * @param bMarked
   ...
   * @param aDeadwoodDoD
   *        Totholzmengen aufgeteilt in die Zersetzungsklassen. May not be
   *        <code>null</code>.
  ...
  {
    // Create new object
    final ExBiotopbaumBO aExBiotopbaumBO = new ExBiotopbaumBO (nBBNr,
                                                               aPics,
                                                               aDate,
                                                               aType,
                                                               sLocation,
    ...
    // Success audit
    AuditHelper.onAuditCreateSuccess (ExBiotopbaumBO.OT,
                                      aExBiotopbaumBO.getID (),
                                      Integer.valueOf (nBBNr),
                                      aPics,
                                      aDate,
                                      aType,
                                      sLocation,
                                      eExposition,
                                      sHanglage,
                                      Boolean.valueOf (bEinschichtig),
                                      Boolean.valueOf (bSolitary),
                                      Boolean.valueOf (bLightLocation),
                                      Boolean.valueOf (bClosedCrown),
                                      Boolean.valueOf (bNoSun),
                                      Boolean.valueOf (bUeberSun),
                                      Boolean.valueOf (bHomogene),
                                      sBeschreibung,
                                      eTreeKind,
                                      aCaves,
                                      aTrunk,
                                      ...
                                      aSpecialStructure,
                                      sOtherSpecial,
                                      Boolean.valueOf (bAspirant),
                                      sAspirantDesc,
                                      Boolean.valueOf (bMarked),
                                      sMarkedDesc,
                                      aDeadwoodCats,
                                      aDeadwoodDoD);
                                             ...
                                             @Nonnull final ICommonsList <IExBiotopbaumDecompositionDegreeBO> aDeadwoodDoD)
  {
    ...
    // Success audit
    AuditHelper.onAuditModifySuccess (ExBiotopbaumBO.OT,
                                      "all",
                                      aExBiotopbaumBO.getID (),
                                      Integer.valueOf (nBBNr),
                                      aPics,
  ...
  /**
   * Mark an existing object as deleted. This means the object is still present
   * and can be restored using the {@link #markExBiotopbaumBOUndeleted(String)}
   * method.
   * 
   * @param sExBiotopbaumBOID
   *        ID of the object to be marked as deleted. May be <code>null</code>.
   * @return {@link EChange#CHANGED} if the object was marked as deleted
   *         {@link EChange#UNCHANGED} if the object does not exist or was
   *         already deleted. Never <code>null</code>.
   */
  @Nonnull
  ...
  /**
   * Restore an existing object that was marked deleted using the
   * {@link #markExBiotopbaumBODeleted(String)} method.
   * 
   * @param sExBiotopbaumBOID
   *        ID of the object to be marked as undeleted. May be
   *        <code>null</code>.
   * @return {@link EChange#CHANGED} if the object was undeleted
   *         {@link EChange#UNCHANGED} if the object does not exist or was not
   *         deleted. Never <code>null</code>.
   */
  @Nonnull
   ...
   * Note: if an object was previously marked as deleted it can finally be
   * deleted with this method.
   * 
   * @param sExBiotopbaumBOID
   *        ID of the object to be deleted. May be <code>null</code>.
   * @return {@link EChange#CHANGED} if the object was deleted
   *         {@link EChange#UNCHANGED} if the object does not exist. Never
   *         <code>null</code>.
   */
  @Nonnull
   ...
   * @return The object with the ID or <code>null</code> if no such object is
   *         present.
   */
  @Nullable
